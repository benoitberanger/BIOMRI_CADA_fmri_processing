...
outdir = e.getSerie('run').removeEmpty.toJob(0);
func   = e.getSerie('run').removeEmpty.getVolume('^rv').toJob(0);
% mask   = e.getSerie('run').removeEmpty.getExam.getSerie('anat_T1').getVolume('^rp[23]').toJob(0);
mask   = e.getSerie('run').removeEmpty.getExam.getSerie('anat_T1').getVolume('^rp[2]').toJob(0);

e.getSerie('run').addJson('^rp.*txt$','rp',1)
rp = e.getSerie('run').removeEmpty.getJson('rp').toJob(0);
...
% par.noiseROI_n_voxel_crop = [1 1];           % crop n voxels in each direction, to avoid partial volume
par.noiseROI_thresholds   = 0.95;     % keep voxels with tissu probabilty >= 95%
par.noiseROI_n_voxel_crop = 1;           % crop n voxels in each direction, to avoid partial volume
par.noiseROI_n_components = 10;              % keep n PCA componenets
par.output_filename = 'multiple_regressors__RP_FD.txt';
